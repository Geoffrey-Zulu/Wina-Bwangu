// Long Json data from Appendix, keep minimized
...
function calculateAndDisplayCumulativeRevenue() {
  ...
  historicalData.forEach((transaction) => {
    ...
    const revenue =
      transaction.RevenuePerKwacha * transaction.TransactionAmount;
  ...
  const cumulativeRevenueResultElement =
    document.getElementById("totalsResult");
...
document
  .getElementById("cumulativeRevenue")
  .addEventListener("click", calculateAndDisplayCumulativeRevenue);




  // Function the calculate and display totals
function calculateAndDisplayTotals() {
  ...
  historicalData.forEach((transaction) => {
    ...
    // Calculate tax (can be replaced with any tax rate)
  ...
  totalsResultElement.innerHTML = `
        ...
        <p>Total Transaction Amount After Tax: ${totalTransactionAfterTax.toFixed(
          2
        )} Kwacha</p>
    `;
}
...
document
  ...
  .addEventListener("click", calculateAndDisplayTotals);




  // Function for the remaining from monthly limit
function calculateAndDisplayRemaining() {
  // Monthly limit has been increased because all the data was used in the appendix
  const monthlyLimits = {
    "Airtel Money": 350000,
    "MTN Money": 160000,
    "Zamtel Money": 90000,
...
document
  .getElementById("calculateRemaining")
  .addEventListener("click", calculateAndDisplayRemaining);
  ...
  // Goolge charts loading and logic
...
function populateServiceFrequencyTable() {
  ...
  // Loop through historical data to calculate service frequency
  historicalData.forEach((transaction) => {
    const mobileBooth = transaction.MobileBooth;
    const service = transaction.Service;

    // Initialize the count for the mobile booth if not already done
    if (!serviceFrequencyData[mobileBooth]) {
      serviceFrequencyData[mobileBooth] = {};
    }

    // Increment the count for the service at the mobile booth
    if (!serviceFrequencyData[mobileBooth][service]) {
      serviceFrequencyData[mobileBooth][service] = 1;
    } else {
      serviceFrequencyData[mobileBooth][service]++;
    }
  });

  // Populate the table with service frequency data
  for (const mobileBooth in serviceFrequencyData) {
    if (serviceFrequencyData.hasOwnProperty(mobileBooth)) {
      const services = serviceFrequencyData[mobileBooth];
      for (const service in services) {
        if (services.hasOwnProperty(service)) {
          const frequency = services[service];
          const row = serviceFrequencyTable.insertRow();
          const boothCell = row.insertCell(0);
          const serviceCell = row.insertCell(1);
          const frequencyCell = row.insertCell(2);

          boothCell.textContent = mobileBooth;
